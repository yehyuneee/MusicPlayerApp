...
@Dao
interface LyricsDAO {
    @Query("SELECT * FROM lyricsList")
    fun getAll(): LiveData<List<LyricsEntity>>

    @Query("SELECT lyrics FROM lyricsList WHERE duration = :currentDuration")
    fun getLyrics(currentDuration: Long): String

    @Insert(onConflict = OnConflictStrategy.IGNORE)
    fun insert(lyrics: LyricsEntity)
}